#!/usr/bin/python3
# -*- coding: utf-8 -*-
# PiLogger Web-Monitor DE, Version 0.10 beta, 2020-07-02
# Coypright 2018,2020 G.Weiß-Engel
#
...
Avg[V];Min[V];Max[V]; Mom [A]; Avg [A]; Min [A]; Max [A]; Mom [W]; Avg [W]; Min [W]; Max [W]; Ohm [Ω] ; Bila [Wh];\
 Ertr [Wh]; Verb [Wh];BilTg [Wh];ErtTg [Wh];VerTg [Wh]'.format(UnitPulse1)
	lineB = str(datetime.now())							# os.path.getmtime(path) ?
	lineB = lineB.split('.',1)[0]						# microsec abtrennen
	lineB += ';NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN;NaN'
	with open(WorkDir+'/logdata.csv','a') as datafile:
		print(lineA,file=datafile)						# 268 Zeichen pro Zeile
...
def SplitLogfile():
	try:
		copyfile('/home/pi/logdata.csv','/home/pi/showdata.csv')
	except IOError as e:
...
def getPiLoTemp():
	...
	if tc != '-UTS-':
		tf = tc * 1.8 + 32
		out1 = '{0:-6.1f} °C'.format(tc)
		out1 = out1.replace('.',',')
		out2 = '{0:-6.1f} °F'.format(tf)
		out2 = out2.replace('.',',')
	else:
		out1 = tc
		out2 = tc
...
def getPiLoWind():
	...
	try:
		...
		ant = 0
		Fehler = getLogTime()+" I2C Fehler @ Wind-Lesen;"
	res1 = ant * FactPulse1 / MeasInter / TimPuFact
	out1 = '{0:-6.1f} {1}'.format(res1,UnitPulse1)
	out1 = out1.replace('.',',')
	res2 = ant * FactPulse2 / MeasInter / TimPuFact
	out2 = '{0:-6.1f} {1}'.format(res2,UnitPulse2)
	out2 = out2.replace('.',',')
	return out1,out2,Fehler
...
def getPiLoVoltAmp():
	...
	out1 = '{0:-8.3f} V'.format(volt)
	out1 = out1.replace('.',',')
	out2 = '{0:-8.3f} A'.format(amp)
	out2 = out2.replace('.',',')
	out4 = 'Laden'
	...
	if volt < 0:
		volt = -volt
	if amp < 0.0004:									# Division durch Null vermeiden
		amp = 0.0002
	ohm = volt / amp									# R = U / I
	if ohm > 999:										# auf 1 kOhm begrenzen
		ohm = 999
	out3 = '{0:-7.1f} Ω'.format(ohm)
	out3 = out3.replace('.',',')
	out5 = '{0:-8.3f} W'.format(aktP)
	out5 = out5.replace('.',',')
	return out1,out2,out3,out4,out5,Fehler
...
def dotheLog():
	...
	if DoVAcorr:
		...
		ampavg = ampavg * CorrAmpFactor + CorrAmpOffset
	if voltavg < 0:										# Negative Spannung nur durch Korrektur möglich
		voltavg = - voltavg								# Betrag bilden
		avgP = - avgP									# Fehlzählung durch negative Spannung nach Korrektur verhindern
	if ampavg < 0:
		ampavg = -ampavg								# Betrag bilden
	if ampavg < 0.0004:									# Division duch Null vermeiden
		ampavg = 0.0002
	ohm = voltavg / ampavg
	if ohm > 999:										# auf 1000 Ohm begrenzen
		ohm = 999
	line = line+';'+'{0:-9.3f}'.format(ohm)
...
def download(filename):
	if filename == 'logdata.csv':
		...
		try:
			copyfile('/home/pi/logdata.csv','/home/pi/logdata_snap.csv')
...
def Write_PiLoConf():
	global MeasInter, I2Cadr, TimPuFact, MeasCount
...
def Req_new_data():
	...
	data["PiLoWind1"],data["PiLoWind2"],Fehler = getPiLoWind()
	if Fehler != "":
		FehlerMeldung += Fehler
	data["PiLoVolt"],data["PiLoAmps"],data["PiLoOhms"],data["PiLoMode"],data["PiLoWatt"],Fehler = getPiLoVoltAmp()
	if Fehler != "":
		FehlerMeldung += Fehler
	data["TgEner"] = getPiLoEner(EnerCntTagB)
	data["TgEnTi"] = getPiLoEnTi(TimeCntTag)
	data["TgEnerE"] = getPiLoEner(EnerCntTagE)
	data["TgEnTiE"] = getPiLoEnTi(TimeCntTag)
	data["TgEnerV"] = getPiLoEner(EnerCntTagV)
	data["TgEnTiV"] = getPiLoEnTi(TimeCntTag)
	data["PiLoEner"] = getPiLoEner(EnerCntB)
	data["PiLoEnTi"] = getPiLoEnTi(TimeCount)
	data["PiLoEnerE"] = getPiLoEner(EnerCntE)
	data["PiLoEnTiE"] = getPiLoEnTi(TimeCount)
	data["PiLoEnerV"] = getPiLoEner(EnerCntV)
	data["PiLoEnTiV"] = getPiLoEnTi(TimeCount)
...
GPIO.add_event_detect(4,GPIO.FALLING,JetztAber)
