#!/usr/bin/python3
# -*- coding: utf-8 -*-
# PiLogger Monitor DE, Version 1.2, 2018-01-17
# Coypright 2017,2018 G.Weiß-Engel
#
...
Mom [V];Avg [V];Min [V];Max [V];Mom [A];Avg [A];Min [A];Max [A];Mom [W];Avg [W];Min [W];Max [W]'.format(UnitPulse)
  ...
  def poll(self):
    ...
    try:
      ...
      messagebox.showerror("Fehler","Fehler I²C Bus bei BlockRead")
    ...
    for i in range (16):
      z=vl.pop(0)
      if i < 4:                                   # Temperatur
        if TempSense == 'NTC 10k':
          v = z / 65535
          v = v * 1.016                           # * Korrektur Exemplarstreuung
          if z < 1763:
            w = 999
          elif z < 8424:
        ...
        elif TempSense == 'PT1000':
          v = z * 0.985 - 220                     # * Korrektur Exemplarstreuung
          if z < 25000:
            w = -999
          elif z < 50000:
            w = (v*v)/23006845.6+v/167.473355-237.0911
          else:
            w = 999
          out = '{0:-7.2f} °C'.format(w)
          if dologg:
...
class einstell(Toplevel):
  ...
  def body(self, master):
    ...
    self.cb1 = ttk.Combobox(master,textvariable=newTimeBase,values=(0,1,2,3),state='readonly',font=(PiLoFont,12),width=5,justify=CENTER)
    ...
    self.cb2 = ttk.Combobox(master,textvariable=newAvgFactr,values=(0,1,2,3,4,5,6,7),state='readonly',font=(PiLoFont,12),width=5,justify=CENTER)
    ...
    self.cb3 = ttk.Combobox(master,textvariable=TempSens,values=("NTC 10k","PT1000"),state='readonly',font=(PiLoFont,12),width=8,justify=CENTER)
